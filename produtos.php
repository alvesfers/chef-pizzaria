...
if (
    !$usuarioLogado
    || !in_array($usuarioLogado['tipo_usuario'], ['admin', 'funcionario'])
) {
...
$subcats = $pdo
    ...
    INNER JOIN tb_subcategoria_categoria scc
            ON scc.id_subcategoria = sc.id_subcategoria
         WHERE sc.subcategoria_ativa = 1
      ORDER BY scc.id_categoria, sc.nome_subcategoria
    ")
    ->fetchAll(PDO::FETCH_ASSOC);

// 3) Produtos (listagem inicial)
$produtos = $pdo
    ->query("
...
// 4) Tipos de Adicional por Categoria (sem obrigatoriedade nem limites)
$tipoCat = $pdo
    ->query("
        SELECT tac.id_tipo_adicional,
               tac.id_categoria,
...
// 5) Adicionais (todos, agrupados por tipo)
$adicionais = $pdo
    ->query("
        SELECT id_adicional,
               id_tipo_adicional,
...
<div class="container mx-auto p-4">
    ...
    <div class="overflow-x-auto mb-4">
        <table class="table w-full">
            <thead>
                <tr>
                    <th class="sortable" data-field="nome_produto">
                        Nome <span class="sort-indicator"></span>
                    </th>
                    <th class="sortable" data-field="nome_categoria">
                        Categoria <span class="sort-indicator"></span>
                    </th>
                    <th class="sortable" data-field="valor_produto">
                        Preço <span class="sort-indicator"></span>
                    </th>
                    <th class="sortable" data-field="produto_ativo">
                        Ativo <span class="sort-indicator"></span>
                    </th>
                    <th>Ações</th>
                </tr>
            </thead>
            <tbody id="product-table-body">
                <!-- preenchido via produtos.js usando window.__produtos__ -->
            </tbody>
        </table>
    </div>
        ...
        <form id="product-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="pf-nome" name="nome_produto"
                        class="input input-bordered w-full" required />
                </div>
                <div>
                    <label class="block font-medium mb-1">Preço</label>
                    <input type="text" id="pf-valor" name="valor_produto"
                        class="input input-bordered w-full currency" required />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div class="md:col-span-2">
                    ...
                    <textarea id="pf-descricao" name="descricao_produto"
                        class="textarea textarea-bordered w-full" rows="1"></textarea>
                </div>
                <div>
                    <label class="block font-medium mb-1">Categoria</label>
                    <select id="pf-categoria" name="id_categoria"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select id="pf-has-inclusos" name="has_inclusos"
                        class="select select-bordered w-full">
                        <option value="0">Não</option>
                        <option value="1">Sim</option>
                    </select>
